...
def _build_prompt(trip, regions, excluded_places, *, total_override: int | None = None):
    ...
    prompt = f"""You are a travel expert for India. A user is planning a trip.

EXPLORATION SCOPE (STRICT CONSTRAINT):
- Regions to explore: {regions_str}
- All suggested places MUST be located within these regions: {regions_str}.
- The "region" field in your JSON response MUST be EXACTLY one of: {regions_str}.
- **REGIONAL BALANCE (CRITICAL)**: Ensure NO single region dominates. Distribute places across ALL listed exploration regions roughly equally (~{per_region} per region).

LOGISTIC PREFERENCES (PREFERENCE ONLY):
- Trip Start (Logistics): {start_region or 'Not specified'}
- Trip End (Logistics): {end_region or 'Not specified'}
- IMPORTANT: These are for arrival/departure logistics ONLY. Do NOT suggest points of interest in these regions unless they are also listed in "Regions to explore". 
- Even if the user starts in {start_region}, if {start_region} is NOT in the "Regions to explore" list, do NOT suggest ANY place from {start_region}. 
- Focus 100% of your sightseeing suggestions on the "Regions to explore".
...
INSTRUCTIONS:
- Suggest exactly {total} real, visitable places strictly within the specified "Regions to explore".
...
- Avoid non-specific activities/itineraries like "boat ride", "safari", "food tour", or generic area names like "Explore Old Town".
- For each place, provide: name, category, the region name (from the allowed list), and a SHORT 1-sentence description (max 15 words).
...
- Distribute places across the explore regions roughly equally (~{per_region} per region).
{mode_instruction}
{companion_instruction}
{season_instruction}
- Do NOT invent new regions or alternate spellings. Only use: {regions_str}.
- Ensure all places are REAL and currently open to tourists in India.
- Avoid informal names or hyper-local nicknames.
...
{{
  "suggestions": [
    {{"name": "Official Place Name", "category": "category", "region": "region_name", "description": "Engaging 1-sentence description..."}},
    ]
}}"""
